...
class Server {
    ...
    private fun closeServer() {
        for (userSocket in activeUsers.keys){
            sendMessage(userSocket, "/goodbye")
            userSocket.close()
        }
        ...
        try {
            ...
            if (userBase.hasUser(login)) {
                ...
                if (userBase.checkPassword(login, pass)) {
                    ...
                    sendMessage(clientSocket, "Приветствуем вас.")
                    return true
                } else {
                ...
                while (true) {
                    ...
                    activeUsers.put(clientSocket, login)
                    sendMessage(clientSocket, "Успешно.\n" +
                            "Приветствуем вас.")
                    return true
...
//            return UserAction.HELP
    ...
    private fun handleSay(clientSocket: Socket, login: String, message: String) {
        ...
        //Мы с коллегой решили, что нет смысла посылать клиенту ответ в данной ситуации,
        //однако саму функцию решили оставить.
...
//        sendMessage(clientSocket, "Ваше сообщение доставлено.")
    ...
    private fun handleSqrt(clientSocket: Socket, sqrt: String) {
        Thread(Runnable {
            ...
            sendMessage(clientSocket, "s $response")
    ...
    private fun handleFact(clientSocket: Socket, factorial: String) {
        Thread(Runnable {
            ...
            //качество доставки, даже если посылка будет очень большой
...
//            }
    ...
    private fun handleEval(clientSocket: Socket, expression: String) {
        ...
        if (response != "Не удалось вычислить выражение."){
            addHistory(getName(clientSocket), "EVAL: $expression = $response")
        }
        sendMessage(clientSocket, "e $response")
    }
