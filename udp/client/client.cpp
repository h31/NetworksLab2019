...
void server_connect(){
    ...
    if (sendto(socket_descriptor, &temp, sizeof(int), 0, (struct sockaddr *) &server_address, sizeof(server_address)) < 0) {
        exit(1);
    }
    if (sendto(socket_descriptor, &nickname, temp, 0, (struct sockaddr *) &server_address, sizeof(server_address)) < 0) {
        exit(1);
    }
}
...
void send_message(){
    ...
    int temp = sendto(socket_descriptor, (char *) &message_length, sizeof(int), 0, (struct sockaddr *) &server_address, sizeof(server_address));
    if (temp <= 0) {
        ncurses_close();
        perror("ERROR writing to socket");
        exit(1);
    }
    int temp = sendto(socket_descriptor, (char *) &output_buffer, message_length, 0, (struct sockaddr *) &server_address, sizeof(server_address));
    if (temp <= 0) {
        ncurses_close();
        perror("ERROR writing to socket");
        exit(1);
...
void read_server_response(){
    ...
    int temp = recvfrom(socket_descriptor, (char*) message_size, sizeof(int), 0, (struct sockaddr *) &server_address, &size_addr);
    if (temp <= 0) {
        ncurses_close();
    ...
    size_addr = sizeof(server_address);
    int temp = recvfrom(socket_descriptor, (char*) input_buffer, message_size, 0, (struct sockaddr *) &server_address, &size_addr);
    seconds = time(0);
    if (temp <= 0) {
        ncurses_close();
        perror("ERROR reading from socket");
