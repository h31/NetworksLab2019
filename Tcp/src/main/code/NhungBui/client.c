...
#include
#include
#include #include #include #include #include #include #define DEF_PORT 6789
...
{
    int sock = (int) arg, sendAuth = 0, flag = 0;
    char msg[msg_length];
    for(;;)
    {
        if (!sendAuth && !flag)
        {
            printf(CYN "Введите имс пользователс [a-zA-Z0-9] от 5 до 15 символов:\n"RESET);
            flag = 1;
        }
        else if (sendAuth && flag)
        {
            printf(CYN"Придумайте пароль [a-zA-Z0-9] от 5 до 15 символов:\n"RESET);
            flag = 0;
        }
        ...
        if (sendAuth < 2)
        {
            ...
            {
                printf (CYN "сеправильный формат" RESET "\n");
                continue;
            }
        ...
        else
        {
            ...
            {
                printf(CYN "сеизвестнас команда" RESET "\n");
                continue;
            }
        }
...
void printMenu()
{
    ...
    printf("eval | Считает значение выраженис\n");
    printf("fact | Считает указанный факториал целого числа\n");
    printf("sqrt | Считает квадратный корень из указанного числа\n");
    printf("say | Отправить сообщение серверу\n");
...
void* rcvHandler(void *arg)
{
    ...
    for(;;)
    {
        ...
        switch(msg[0])
        {
            ...
            case 's':
            case 'S':
                printf(CYN "� езультат операции: " RESET);
                flag = 1;
                break;
            case 'h':
            case 'H':
                printf(CYN "Историс ваших запросов: " RESET "\n");
                flag = 1;
                break;
            case 'o':
            case 'O':
                printf(CYN "Операторы, доступные в рамках комманды eval:" RESET "\n");
                flag = 1;
                break;
            default:
                break;
...
int readFix(int sock, char* buf, int bufSize, int flags)
{
// читаем "заголовок" - сколько байт составлсет наше сообщение
